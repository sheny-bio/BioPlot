...
print(paste0("Numbers of KEGG enrichment:",dim(kegg)[1]))
print(paste0("Numbers of GO_BP enrichment:",dim(go_bp)[1]))
print(paste0("Numbers of GO_MF enrichment:",dim(go_mf)[1]))
print(paste0("Numbers of GO_MF enrichment:",dim(go_mf)[1]))


a<-paste0("Numbers of KEGG enrichment:",dim(kegg)[1])
b<-paste0("Numbers of GO_BP enrichment:",dim(go_bp)[1])
c<-paste0("Numbers of GO_MF enrichment:",dim(go_mf)[1])
d<-paste0("Numbers of GO_MF enrichment:",dim(go_mf)[1])
...
for(iterm in c(1:length(enrich_list))){
  ...
  if(dim(deduplicate.pathway.df)[1]==0){
    ...
    if(p_module==1){
      ...
      p <- ggplot(deduplicate.pathway.df, aes(x = Cluster, y = Description)) +
        ...
        xlab('') +
        ylab('') +
        labs(size='GeneRatio', colour='pvalue') +
        scale_x_discrete(drop = F)
      ggsave(filename = out_plot_name, width = as.numeric(my_width), height = as.numeric(my_height), limitsize = F)
    }
  ...
  write.table(logfile,"workLOG.txt",sep = "\t",col.names = F,row.names = F,quote = F,append = TRUE)
